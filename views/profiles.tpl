...
<body>
  ...
  <table class="table" width="100%" border="2" >
    ...
    <tr>
      <!-- Logo -->
      <td bgcolor="2C3E50">
        <img src="/logo_proxy/{{row[0]}}" onerror="this.onerror=null;this.src='/logo/0000.png';" loading="lazy" decoding="async" style="border:0;width:45%;" >
      </td>
      <!-- Enabled -->
      <%
        ids = 'cnl_' + row[0]
  ...
  <script>
    ...
    function goPage(usrName) {
        page = document.getElementById('go_page').value;
        window.location.href = "/profile/" + usrName + "/" + page;
    }
  </script>
    ...
    <div class="container justify-content-center"> <button class="navbar-toggler navbar-toggler-right border-0" type="button" data-toggle="collapse" data-target="#navbar_bottom">
      ...
      <div class="collapse navbar-collapse text-center justify-content-center" id="navbar_bottom">
        <ul class="navbar-nav">
          ...
          % if last_page > 2:
            &nbsp;&nbsp;&nbsp;
            <select class="form-control" id="go_page" onChange="goPage('{{usrName}}')" >
                <option>go to</option>
